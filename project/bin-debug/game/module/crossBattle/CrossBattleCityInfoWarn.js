...
var CrossBattleCityInfoWarn = (function (_super) {
    ...
    CrossBattleCityInfoWarn.prototype.UpdateContent = function () {
        ...
        this.text.text = data.camp == 0 ? "Empire" : "Frontier";
    ...
    CrossBattleCityInfoWarn.prototype.onTap = function () {
        ...
        if (this.currentState == "gong") {
            if (GameGlobal.CrossBattleModel.status == 1) {
                UserTips.ErrorTip("Cannot attack in preparation phase");
                return;
            }
            if (GameGlobal.CrossBattleModel.status == 3) {
                UserTips.ErrorTip("Cannot attack while dead");
                return;
            }
            if (GameGlobal.CrossBattleModel.status == 4) {
                UserTips.ErrorTip("Cannot attack defenders");
                return;
            }
            ...
            if (GameGlobal.CrossBattleModel.status == 1) {
                UserTips.ErrorTip("Cannot defend in preparation phase");
                return;
            }
            if (GameGlobal.CrossBattleModel.status == 3) {
                UserTips.ErrorTip("Cannot defend while dead");
                return;
            }
            if (GameGlobal.CrossBattleModel.status == 4) {
                UserTips.ErrorTip("Cannot defend while defending");
                return;
            }
