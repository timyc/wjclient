class CatchPetResultWin extends ResultBasePanel {
	...
	OnOpen(...param: any[]) {
		this.SetBtnLabel("Attempt")
		this.SetTitleLabel("Capture Success")
		...
		this.text.text = 'Congratulations for obtaining ' + GameGlobal.Config.petBiographyConfig[param[0]].name
		if (star != null) {
			if (star == 0) {
