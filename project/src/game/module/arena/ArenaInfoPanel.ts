class ArenaInfoPanel extends BaseView implements ICommonWindowTitle {
		...
		switch (e.target) {
			...
			case this.buyBtn:
				...
				WarnWin.show('Use |C:0xfff01e&T:' + GameGlobal.Arena.getPKCountBuy() + 'diamonds |to buy |C:0xd27701&T:' + GameGlobal.Arena.getAddPKCount() + 'attempts\n\n'
					+ 'Remaining times: ' + this.reaminCount + '\n\n' + 'VIP' + GameGlobal.actorModel.vipLv + ' can buy ' + maxCount + ' times daily',
					this.okBuyHandler, this, null, null, "normal", { title: 'Purchase Tips' });
				break
		}
	}
	...
	private getArenaBuyResult(data: Sproto.cs_buy_pk_response): void {
		if (!data.ret) {
			...
			else UserTips.ins().showTips('Insufficient diamonds!');
		}
		else {
			this.pkCount = data.pkcount;
			this.buyCount = data.buycount;
