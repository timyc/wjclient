...
class TianshilianPanel extends BaseEuiView implements ICommonWindow, ICommonWindowTitle {
	...
	public static readonly NAME = "Essences"
	...
	public OnOpen() {
		...
		this.checkBox.labelDisplay.text = "Auto Battle"
	...
	public upDataSelectRect() {
		...
		if (this.nDownTime >= this.DOWNTIME || this.nDownTime < 0) {
			...
			str = this.nDownTime + "S to auto battle"
		}
	...
	public upDateInfo() {
		...
		if (this.tServer.layer > 0) {
			let strPassLv = `|C:0x6E330B&T:Highest Floor: |C:0x019704&T:${this.tServer.layer}|`
			//显示当前最大通关关数
	...
	public upDataLyCn(_item, _index) {
		...
		for (const item in this.tNowLyData) {
			if (item === _index) {
				...
				_item.layerLabel.text = "Floor " + nNowLv
	...
	public updateRewardCn(_item, _index) {
		...
		if (pRewardData[_index]) {
			_item.visible = true
			_item.layerLabel.text = "Floor " + pRewardData[_index].id
	...
	public onClickRw(e: egret.TouchEvent) {
		...
		if (pData) {
			...
			WarnWin.showReward(`|C:0x6E330B&T: Pass |C:0x4fcd4c&T:${strIndex}|C:0x6E330B&T: and obtain|`,
				this.getRewrd, this, null, null, "reward", { title: 'Rewards', reward: pData.firstAward, btnName: "Claim", bHideSureBtn: !bHaveGet });
	...
	public _OnClick(e: egret.TouchEvent) {
		switch (e.currentTarget) {
			case this.oneKeyBtn:
				...
				{
					UserTips.ins().showTips("VIP"+ GameGlobal.Config.HeavenFbBaseConfig.viplv + " can blitz")
				}
				else
				{
					if (this.tServer.todayLayer < this.tServer.layer) {
						...
						UserTips.ins().showTips("Unable to blitz")
					}
				}
				break
