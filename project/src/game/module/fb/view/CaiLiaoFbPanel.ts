class CaiLiaoFbPanel extends BaseView implements ICommonWindowTitle {
	public static readonly NAME = "Materials";
...
class CaiLiaoFBItem extends eui.ItemRenderer {
	...
	public onTap(e) {
		if (!UserFb.FinishAndCheckFighting()) {
			...
			if(this.data.vipBuyCount<maxExSwap)
			{
				...
				{
					UserTips.ins().showTips("Not enough diamonds")
				}
			}
			else
			{
				...
				UserTips.ins().showTips("VIP"+nNextVip + " get "+nNextTime+" more tries")
			}
	...
	dataChanged(): void {
		...
		if (GameLogic.ins().actorModel.level < config[data.fbID].levelLimit) {
			...
			this.tipsTxt2.textFlow = TextFlowMaker.generateTextFlow(`|C:0x6E330B&T: Level |C:0x4fcd4c&T:${config[data.fbID].levelLimit}|C:0x6E330B&T: needed|`);
			...
			if (config[data.fbID].needsuccess > data.totalCount) {
				if (data.useCount >= config[data.fbID].freeCount) {
					this.tipstxt1.textFlow = TextFlowMaker.generateTextFlow(`|C:0x6E330B&T: VIP|C:0x6E330B&T:${nNextVip}|C:0x6E330B&T: can blitz x|C:0x4fcd4c&T:${nNextTime}|`);
					this.saodangBtn.labelDisplay.text = "Blitz";
					this.redPoint.visible = false;
					this.moneyTxt.visible = true;
					this.moneyTxt.text = config[data.fbID].buyPrice[data.vipBuyCount];
				} else {
					this.tipstxt1.textFlow = TextFlowMaker.generateTextFlow(`|C:0x6E330B&T: Today's attempts: |C:0x4fcd4c&T:${(config[data.fbID].freeCount)}|`);
					this.saodangBtn.labelDisplay.text = "Fight";
				...
				if (data.useCount >= config[data.fbID].freeCount) {
					...
					this.tipstxt1.textFlow = TextFlowMaker.generateTextFlow(`|C:0x6E330B&T:Today's blitz attempts: |C:0x4fcd4c&T:${config[data.fbID].freeCount}|`);
					this.saodangBtn.labelDisplay.text = "Free Blitz";
					this.redPoint.visible = true;
					this.moneyTxt.visible = false;
				}
			}
