...
class FbCbtPanel extends BaseView implements ICommonWindowTitle {
	...
	public static readonly NAME = "Treasure"
	...
	public OnOpen() {
		...
		this.checkBox.labelDisplay.text = "Auto Fight"
	...
	public upDataSelectRect() {
		...
		if (this.nDownTime >= 10 || this.nDownTime < 0) {
			...
			str = this.nDownTime + "S until auto"
		}
		...
		switch (e.target) {
			case this.oneKeyBtn:
				...
				if (GameGlobal.UserFb.getCbtCanSwap()) {
					...
					UserTips.ins().showTips("Cannot Blitz")
				}
				break
	...
	public onClickItem(e: egret.TouchEvent) {
		...
		var strIndex = "Stage " + this.nIndex
		var strLv = pData.star + " Stars"
		...
		WarnWin.showReward(`|C:0x6E330B&T: Complete |C:0x4fcd4c&T:${strIndex}|C:0x6E330B&T: to get |C:0x4fcd4c&T:${strLv}|C:0x6E330B&T: reward|`,
			this.getRewrd, this, null, null, "reward", { title: 'Rewards', reward: pData.starAward, btnName: "Claim", bHideSureBtn: !bHaveGet });
	...
	public updateInfo() {
		var pData = this.getNowLvData()
		this.lbTitle.text = "Treasure " + this.nIndex//页数

		this.lbLv.text = "Stage " + pData.id //关卡
		...
		this.lbVip.text = "VIP" + GlobalConfig.ins().TreasureMapBaseConfig.viplevel + " Unlock"
