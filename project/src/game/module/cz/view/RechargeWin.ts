class RechargeWin extends BaseEuiView {
	...
	initUI() {
		...
		this.commonWindowBg.SetTitle("Recharge")
	...
	public UpdateContent() {
		if (GameGlobal.actorModel.vipLv < RechargeModel.MAX_VIPLV) {
			...
			this.nextVIPTxt.textFlow = TextFlowMaker.generateTextFlow("Recharge " + '|C:0xFFED21&T:' + rechargeRMB + '|' + " more dollars to become " + '|C:0xFFED21&T:VIP' + (GameGlobal.actorModel.vipLv + 1) + '|')
			CommonUtils.addLableStrokeColor(this.nextVIPTxt, 0x460009, 3);
		} else {
			this.bar.maximum = UserVip.ins().exp;
			this.bar.value = UserVip.ins().exp;
			this.nextVIPTxt.text = "VIP Level Max"
		}
		this.bar.value = UserVip.ins().exp;
...
class RechargeListItem extends eui.ItemRenderer {
	...
	public dataChanged() {
		...
		this.RMBTxt.text = '$' + this.data.cash;
